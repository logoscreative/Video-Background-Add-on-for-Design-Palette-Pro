...
//********************************************************************************************************************************
// media uploader for video-background file (mp4)
//********************************************************************************************************************************
  jQuery('div.gppro-video-background-mp4-input').divExists(function () {
    // Uploading files
    var file_frame;
    // our click action
    jQuery('div.gppro-video-background-mp4-input').on('click', 'input.gppro-video-background-mp4-upload', function () {
      ...
      file_frame.on('select', function () {
        ...
        jQuery('input.gppro-video-background-mp4-field').val(userImage);
      });
  ...
  //********************************************************************************************************************************
// media uploader for video-background file (webm)
//********************************************************************************************************************************
  jQuery('div.gppro-video-background-webm-input').divExists(function () {
    // Uploading files
    var file_frame;
    // our click action
    jQuery('div.gppro-video-background-webm-input').on('click', 'input.gppro-video-background-webm-upload', function () {
      // If the media frame already exists, reopen it.
      if (file_frame) {
        file_frame.open();
        return;
      ...
      file_frame.on('select', function () {
        ...
        // check file type to only allow .png .gif or .ico
        if (jQuery.inArray(attachment.subtype, ['webm']) == -1) {
          return;
        }
        // fetch the URL of the attachment grab the relevant data
        userImage = attachment.url;
        // populate the appropriate areas
        jQuery('input.gppro-video-background-webm-field').val(userImage);
      });
      // Finally, open the modal
      file_frame.open();
    });
